...
; - lire 4 octets par 4 octets de sample / attention à l'alignement du sample, lire les premiers octets en loadb et/ou loadw !
...
LSP_DSP_Audio_frequence					.equ			55000				; real hardware needs lower sample frequencies than emulators !
...
; affiche les registres externes
	...
	move.l	LSP_DSP_PAULA_AUD2L_modulo4,d0
	bsr		print_nombre_hexa_8_chiffres
	move.l	#' ',d0
	bsr		print_caractere
	move.l	LSP_DSP_PAULA_AUD2L_current_byte,d0
...
; fin de sample => on recharge les infos des registres externes
		movei		#LSP_DSP_PAULA_AUD3LEN,R22			; fin, a virgule 
		movei		#LSP_DSP_PAULA_AUD3L,R24			; sample location a virgule
		load		(R22),R22
		load		(R24),R26
		store		R22,(R25)							; update internal sample end, a virgule
...
DSP_LSP_routine_interruption_I2S_pas_fin_de_sample_channel3:
		...
		shrq		#nb_bits_virgule_offset,R26								; partie entiere
		load		(R23),R23							; R23 = volume : 6 bits
		loadb		(R26),R21							; R18=sample channel 0 8 bits unsigned
...
; channel 2
		...
		nop
; fin de sample => on recharge les infos des registres externes
		movei		#LSP_DSP_PAULA_AUD2LEN,R22			; fin, a virgule 
...
DSP_LSP_routine_interruption_I2S_pas_fin_de_sample_channel2:
		...
		shrq		#nb_bits_virgule_offset,R26								; partie entiere
		load		(R23),R23							; R23 = volume : 6 bits
		loadb		(R26),R20							; R18=sample channel 0 8 bits unsigned
...
; channel 1
		...
		nop
; fin de sample => on recharge les infos des registres externes
		movei		#LSP_DSP_PAULA_AUD1LEN,R22			; fin, a virgule 
...
DSP_LSP_routine_interruption_I2S_pas_fin_de_sample_channel1:
		...
		shrq		#nb_bits_virgule_offset,R26								; partie entiere
		load		(R23),R23							; R23 = volume : 6 bits
		loadb		(R26),R19							; R18=sample channel 0 8 bits unsigned
		mult		R23,R19								; unsigned multiplication : unsigned sample * volume => 8bits + 6 bits = 14 bits
; R19=sample channel 2 on 14 bits
...
; channel 0
		...
		nop
; fin de sample => on recharge les infos des registres externes
		movei		#LSP_DSP_PAULA_AUD0LEN,R22			; fin, a virgule 
...
DSP_LSP_routine_interruption_I2S_pas_fin_de_sample_channel0:
		...
		shrq		#nb_bits_virgule_offset,R26								; partie entiere
		load		(R23),R23							; R23 = volume : 6 bits
		loadb		(R26),R18							; R18=sample channel 0 8 bits unsigned
...
;--------------------------------------------------
; calculs des modulo 4 et position sur l'octet
	movei		#LSP_DSP_PAULA_AUD2L,R0
	movei		#LSP_DSP_PAULA_AUD2L_modulo4,R1
	movei		#LSP_DSP_PAULA_AUD2L_current_byte,R2
	load		(R0),R0
	movei		#$FFFFFFFC,R4
	move		R0,R3
	moveq		#%11,R5
	and			R4,R3
	store		R3,(R1)						; stocke pointeur modulo 4
	and			R5,R0
	movei		#LSP_DSP_PAULA_AUD2DAT,R6			
	store		R0,(R2)						; stocke le numero de l'octet à lire
	load		(R3),R7						; lire 4 octets alignés
	store		R7,(R6)
...
DSP_LSP_Timer1_r_chgbpm:
	movei		#DSP_LSP_Timer1_process,R12
	loadb		(R0),R11
	store		R11,(R14+(m_currentBpm/4))		; R3=nouveau bpm
...
LSP_DSP_PAULA_AUD0LEN:				dc.l			1<<nb_bits_virgule_offset			; en bytes !
...
LSP_DSP_PAULA_internal_length0:		dc.l			1<<nb_bits_virgule_offset			; internal register : length of the sample currently played
LSP_DSP_repeat_pointeur0:			dc.l			silence
LSP_DSP_repeat_length0:				dc.l			1<<nb_bits_virgule_offset
; channel 1
...
LSP_DSP_PAULA_AUD2L_modulo4:		dc.l			silence			; Audio channel 0 location
LSP_DSP_PAULA_AUD2L_current_byte:	dc.l			5				; current byte to be read
...
LSP_DSP_PAULA_AUD2DAT:				dc.l			0				; bytes in local stock
...
LSP_DSP_PAULA_internal_increment3:	dc.l			15				; internal register : increment linked to period 16:16
